    ...
    <div v-if="user" class="relative">
        ...
        <div
            ...
            class="absolute top-12 right-0 w-48 bg-white border border-slate-200 rounded-lg z-20">
                ...
                <section class="border-t border-b border-slate-200 text-sm text-slate-400 py-2">
                    <router-link
                        :to="{ name: 'user.show', params: { username: user.username }}"
                        @click.native="toggleMenu()"
                        class="flex items-center space-x-3 px-3 py-1 hover:text-slate-500 transition">
