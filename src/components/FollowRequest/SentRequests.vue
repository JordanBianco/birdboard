<template>
    <div>
        <div v-if="sentFollowRequests && sentFollowRequests.length != 0">
            
            <p class="mb-6 text-slate-400 text-sm">Hai inviato {{ sentTotal }} richieste in attesa di risposta.</p>

            <div
                v-for="user in sentFollowRequests"
                :key="user.id"
                class="p-5 rounded-xl shadow-md drop-shadow-xs bg-white dark:bg-zinc-700 shadow-slate-200 dark:shadow-zinc-900 mb-5 last:mb-0 flex justify-between items-start space-x-4">
                    ...
                    <div class="text-xxs text-slate-400 whitespace-nowrap">
                        {{ $moment(user.pivot.created_at).fromNow() }}
                    </div>
            </div>
...
export default {
    ...
    computed: {
        sentPage() {
            return this.$store.state.followRequest.sentPage
        },
        sentLastPage() {
            return this.$store.state.followRequest.sentLastPage
        },
        sentTotal() {
            return this.$store.state.followRequest.sentTotal
        },
    },
    methods: {
        visibilityChanged(isVisible) {
            ...
            if (this.sentPage >= this.sentLastPage) { 
                this.loading = false
                return
            } else {
                this.loading = true

                this.$store.commit('followRequest/NEXT_SENT_PAGE')
